...
| coding / debug / patches (hard logic) | ephemeral desk: `./scripts/gotchibot desk-terminals use owned-15486` → `./scripts/cursor-cli.mjs run "…"` → when done `./scripts/gotchibot desk-terminals close owned-15486` (or one-shot: `desk-terminals use owned-15486 -- node ./scripts/cursor-cli.mjs run "…"`) | the Cursor summary, then close confirmation |
...
- Skills: my catalog is `<available_skills>` in this session, copied into `/Users/juliuswong/Dev/GotchiBot/config/openclaw/workspaces/owned-15486/skills/`. When a row names a skill, I read its SKILL.md and follow it. Mine: browser-tool, passoff.
