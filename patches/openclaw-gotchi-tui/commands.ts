...
import { isGotchiBotEnabled } from "./gotchi-commands.js";
...
type TuiCommandDescriptor = {
  ...
  scope?: "both" | "local" | "remote" | "gotchi";
  shared?: boolean;
  handler?: true;
...
const TUI_COMMAND_ROWS = [
  ...
  [
    "details",
    "Toggle tool/system details (OpenCode-style)",
    "/details",
    undefined,
    { scope: "gotchi" },
  ],
  [
    "orch",
    "Return to GotchiBot orchestrator (avatar + chat)",
    "/orch",
    undefined,
    { scope: "gotchi" },
  ],
  [
    "list",
    "List GotchiBot cAavegotchis and sessions",
    "/list",
    undefined,
    { scope: "gotchi" },
  ],
...
function commandIsVisible(command: TuiCommandDescriptor, local: boolean): boolean {
  if (command.scope === "gotchi") {
    return isGotchiBotEnabled();
  }
