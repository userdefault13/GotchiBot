...
function buildEmptyEditorLine(width: number, paddingX: number): string {
  const maxPadding = Math.max(0, Math.floor((width - 1) / 2));
  const safePadding = Math.min(paddingX, maxPadding);
  const contentWidth = Math.max(1, width - safePadding * 2);
  const layoutWidth = Math.max(1, contentWidth - (safePadding ? 0 : 1));
  const leftPadding = " ".repeat(safePadding);
  const rightPadding = leftPadding;
  return `${leftPadding}${" ".repeat(layoutWidth)}${rightPadding}`;
}
...
export class CustomEditor extends Editor {
  ...
  private minVisibleLines = 1;

  setMinVisibleLines(lines: number): void {
    this.minVisibleLines = Math.max(1, Math.floor(lines));
    this.invalidate();
  }

  override render(width: number): string[] {
    const rendered = super.render(width);
    const min = this.minVisibleLines;
    if (min <= 1 || rendered.length < 2) {
      return rendered;
    }

    let bottomBorderIdx = rendered.length - 1;
    while (bottomBorderIdx > 0 && !isEditorBorderLine(rendered[bottomBorderIdx] ?? "")) {
      bottomBorderIdx -= 1;
    }
    ...
    const content = rendered.slice(1, bottomBorderIdx);
    const paddingX = this.getPaddingX();
    while (content.length < min) {
      content.push(buildEmptyEditorLine(width, paddingX));
    }
    return [rendered[0]!, ...content, ...rendered.slice(bottomBorderIdx)];
  }
