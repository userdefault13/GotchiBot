...
import {
  ...
  isGotchiCollapsingSystemEnabled,
  isGotchiLoadProgressEnabled,
  isGotchiOpencodeChrome,
  isGotchiProseTtsEnabled,
  isGotchiScrollLayout,
  isGotchiTuiMouseEnabled,
  resolveGotchiPromptLines,
  resolveGotchiWheelScrollLines,
  shouldSuppressGotchiSystemLine,
} from "./gotchi-tui-chrome.js";
import { resolveLocalRunShutdownGraceMs } from "./local-run-shutdown.js";
import { editorTheme, resolveEditorTheme, tuiTheme as theme } from "./theme/theme.js";
...
import { createGotchiProseTtsController } from "./gotchi-prose-tts.js";
import { createGotchiLoadProgressController } from "./gotchi-load-progress.js";
...
async function runTuiUnlocked(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const tui: TUI = isGotchiScrollLayout()
    ? new TuiAltScreen(new ProcessTerminal(), undefined, undefined, {
        mouse: isGotchiTuiMouseEnabled(),
        wheelScrollLines: resolveGotchiWheelScrollLines(),
      })
  ...
  const addConnectionNotice = (text: string) => {
    if (isGotchiOpencodeChrome() && shouldSuppressGotchiSystemLine(text)) {
      return;
    }
  ...
  const editor = new CustomEditor(tui, resolveEditorTheme());
  if (isGotchiOpencodeChrome()) {
    editor.setPaddingX(1);
    editor.setMinVisibleLines(resolveGotchiPromptLines());
    editor.borderColor = (line) => theme.accent(line);
  }
  const bottomChrome = new Container();
  bottomChrome.addChild(statusContainer);
  bottomChrome.addChild(footer);
  if (isGotchiOpencodeChrome()) {
    bottomChrome.addChild(new Spacer(1));
  }
  bottomChrome.addChild(editor);

  const loadProgress = createGotchiLoadProgressController(() => tui.requestRender());

  const useScrollLayout = isGotchiScrollLayout() && isViewportTUI(tui);
  let chatScroll: ScrollView | undefined;
  const loadProgressStack =
    loadProgress.component && isGotchiLoadProgressEnabled()
      ? [{ component: loadProgress.component, basis: "auto" as const, shrink: 0 }]
      : [];
  if (useScrollLayout) {
    chatScroll = new ScrollView(chatLog, {
      follow: "end",
      primary: true,
      overscroll: "chain",
      scrollbar: "auto",
    ...
    tui.setLayoutRoot(
      new VStack([
        ...
        ...loadProgressStack,
        { component: chatScroll, basis: 0, grow: 1, minSize: 3 },
        { component: bottomChrome, basis: "auto", shrink: 0 },
      ]),
    );
  } else {
    const root = new Container();
    root.addChild(header);
    if (loadProgress.component) {
      root.addChild(loadProgress.component);
    }
  ...
  const updateHeader = () => {
    ...
    const text = isGotchiOpencodeChrome()
      ? formatGotchiOpencodeHeader({
          agentLabel,
      ...
      : (() => {
          ...
          return `${title} - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`;
        })();
  ...
  const renderStatus = () => {
    ...
    if (
      isGotchiOpencodeChrome() &&
      state.activityStatus === "starting up" &&
      loadProgress.isActive()
    ) {
      statusLoader?.stop();
      statusLoader = null;
      statusText = null;
    ...
    if (isGotchiOpencodeChrome() && !isBusy) {
      statusLoader?.stop();
      statusLoader = null;
      statusText = null;
      statusContainer.clear();
  ...
  const updateFooter = () => {
    const footerText = isGotchiOpencodeChrome()
      ? formatGotchiOpencodeFooter({
          sessionInfo: state.sessionInfo,
    ...
    footer.setText(theme.dim(sanitizeRenderableLine(footerText)));
    if (isGotchiOpencodeChrome()) {
      updateHeader();
    }
  };
  ...
  if (isGotchiProseTtsEnabled() && chatScroll) {
    createGotchiProseTtsController({
      tui,
      chatLog,
      chatScroll,
      header,
      gotchiRoot: process.env.GOTCHIBOT_ROOT?.trim() || process.cwd(),
      openOverlay: (component) => {
        const handle = openOverlay(component);
        return { close: () => closeOverlay(handle) };
      },
      flash: (message) => {
        if (isViewportTUI(tui)) {
          tui.flash(theme.dim(message));
        }
      },
    });
  }
  ...
  editor.onCtrlO = () => {
    if (isGotchiCollapsingSystemEnabled()) {
      // Ctrl+O is reserved for Gotchi orchestrator focus (tmux). Use /details instead.
      return;
    }
  ...
  client.onConnected = () => {
    ...
    loadProgress.set(15, "Gateway connected — subscribing…");
    ...
    void (async () => {
      ...
      loadProgress.set(22, "Subscribed to session…");
      const reconnected = connectionLineage.connect();
      if (reconnected) {
        reconnectStreamingWatchdog();
      }
      loadProgress.set(30, "Loading agents…");
      await refreshAgents();
      if (!ownsConnection()) {
        return;
      }
      loadProgress.set(45, "Restoring session…");
      ...
      updateHeader();
      updateAutocompleteProvider();
      loadProgress.set(60, "Syncing plugins…");
      try {
      ...
      loadProgress.set(72, "Loading task suggestions…");
      try {
  ...
  updateHeader();
  loadProgress.set(5, isLocalMode ? "Starting local runtime…" : "Connecting to gateway…");
