...
import { shouldSuppressGotchiSystemLine, gotchiTurnGapLines, isGotchiCollapsingSystemEnabled, isGotchiOpencodeChrome } from "../gotchi-tui-chrome.js";
import { GotchiSystemTray } from "./gotchi-system-tray.js";
...
export class ChatLog extends Container {
  ...
  private appendNonSystem(component: Component) {
    ...
    if (isGotchiOpencodeChrome()) {
      const gap = gotchiTurnGapLines();
      if (component instanceof UserMessageComponent) {
        if (this.lastTurnKind === "assistant") {
          this.addChild(new Spacer(gap));
        }
        this.lastTurnKind = "user";
      } else if (component instanceof AssistantMessageComponent) {
        if (this.lastTurnKind === "user") {
          this.addChild(new Spacer(gap));
        }
        this.lastTurnKind = "assistant";
      }
    }
  ...
  clearAll() {
    ...
    this.gotchiSystemTray?.clearAll();
    this.gotchiSystemTray = null;
    this.gotchiDetailsVisible = false;
  }
  ...
  addSystem(text: string, opts?: { coalesceConsecutive?: boolean }) {
    if (shouldSuppressGotchiSystemLine(text)) {
      return;
    }
    if (isGotchiCollapsingSystemEnabled()) {
      if (!this.gotchiSystemTray) {
        this.gotchiSystemTray = new GotchiSystemTray();
        this.gotchiSystemTray.setExpanded(this.gotchiDetailsVisible);
        this.append(this.gotchiSystemTray);
      } else {
        this.removeChild(this.gotchiSystemTray);
        this.append(this.gotchiSystemTray);
      }
  ...
  startTool(toolCallId: string, toolName: string, args: unknown, runId?: string) {
    ...
    if (isGotchiCollapsingSystemEnabled()) {
      component.setShowDetails(this.gotchiDetailsVisible);
    }
  ...
  setToolsExpanded(expanded: boolean) {
    this.setGotchiDetailsVisible(expanded);
  }

  setGotchiDetailsVisible(expanded: boolean) {
    this.gotchiDetailsVisible = expanded;
    this.toolsExpanded = expanded;
    for (const tool of this.tools.values()) {
      tool.component.setExpanded(expanded);
      if (isGotchiCollapsingSystemEnabled()) {
        tool.component.setShowDetails(expanded);
      }
    }
    this.gotchiSystemTray?.setExpanded(expanded);
  }

  isGotchiDetailsVisible(): boolean {
