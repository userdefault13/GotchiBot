...
export function createCommandHandlers(context: CommandHandlerContext) {
  ...
  const appendGotchiFocusLines = (result: ReturnType<typeof runGotchiFocus>) => {
    for (const line of formatGotchiFocusOutput(result)) {
      chatLog.addSystem(line);
    }
  };

  const applyGotchiOpenClawSession = async (root: string) => {
    const agentId = readGotchiOpenClawAgentId(root);
    if (!agentId) {
      return;
    }
    await setSession(`agent:${agentId}:main`, agentId);
    chatLog.addSystem(`OpenClaw session → agent:${agentId}:main`);
  };
  ...
  const runGotchiSlash = async (focusArgs: string[], respawnExpected: boolean) => {
    ...
    if (respawnExpected && process.env.TMUX) {
      // agent-focus.mjs respawn-pane -k replaces this TUI; no in-process session switch.
      return;
    }
  ...
  const commandHandlers = {
    ...
    details: async () => {
      const visible = chatLog.toggleGotchiDetailsVisible();
      if (isViewportTUI(tui)) {
        tui.flash(visible ? "details on" : "details off");
      } else {
        chatLog.addSystem(visible ? "details on" : "details off");
      }
      tui.requestRender();
    },
