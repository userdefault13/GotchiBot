...
| Every `abra` call dies with `Cannot find package '<x>' imported from ~/Dev/abracadabra/dist/index.js` | abracadabra's `node_modules` is missing, which silently takes out SSH to the hub, the fleet and every secret. `cd ~/Dev/abracadabra && npm ci` (or `npm install --omit=dev`). A lockfile restore, not a new dependency, so it is allowed |
| Any `~/Dev` project suddenly cannot resolve its own imports | Same cause, wider blast radius: a disk-space cleanup deletes `node_modules` across the whole tree, because ~20 GB of it looks exactly like reclaimable build output. Measured on 2026-09-06: **42 of 45** projects under `~/Dev` had a `package.json` and no `node_modules`. Check `ls ~/Dev/<project>/node_modules` **before** suspecting the host, the tunnel or the hub — a missing dependency tree reads like an outage from every direction |
