...
**Tickets:** `project-tickets` is the request/submit protocol between agents
(request → claim → submit → accept/rework); this kanban is the board the work
shows on. Tickets link to cards via `cardId` — never a second backlog.
