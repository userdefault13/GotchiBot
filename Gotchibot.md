...
| **Spawn / Mint** | Mint new cAavegotchi from 16 starter collaterals via sim pay: **owner free** · **promo whitelist** · **1 free seat per L1 wallet gotchi** *(SIM-side pricing — not enforced by this repo)*; else first `$5`, further `$3`. Or bind-owned from wallet by name (free). Never auto-mint. | Free hero → binds at spawn |
...
- **Mint paths**: available hero → spawn; cartridge miss → mint-sub (16 starters; **owner free** · **promo whitelist** · **1 free per L1 wallet gotchi** *(SIM-side pricing — not enforced by this repo)*, else `$5` then `$3`) or bind-owned (free, by name).
...
## Glossary & roster rules

See **[docs/GLOSSARY.md](docs/GLOSSARY.md)** for Roster vs Crew vs wallet gotchis (unminted),
identity/hub roster, CREWS.md fleet crews, apply-gate rules, and Sepolia bind ABI status.
