...
const ORCH_PROMPT = [
  ...
  "## Claude Code tool (YOU HAVE THIS — named MCP tool claude_ask)",
  "When Julius asks about a Claude tool / @claudemode / Hub Claude pane: answer YES.",
  "Prefer MCP tool claude_ask (server gotchibot-claude). Pass { prompt: \"…\" }.",
  "Fallback Bash if MCP missing (sub-agents: NEVER wrap in abra — Touch ID fails headless):",
  "  node ./scripts/claudemode-ask.mjs \"…\"",
  "  interactive Desk only: abra run gotchibot -- node ./scripts/claudemode-ask.mjs \"…\"",
  "  or: ./scripts/gotchibot claude-ask \"…\"",
  "That hits iMac VS Code Claude Code. Read the reply, then continue the task yourself.",
  "If claude_ask fails (connection refused / bridge down): MCP hub_bridge_ensure or `./scripts/gotchibot hub bridge-ensure`, then retry once. Load hub-sop.",
  ...
  "Skills to load: delegate-first, browser-tool, gotchibot-bridge, hub-sop, synergy.",
