 ...
 *
 * GOTCHIBOT_OPENCODE_IOS / --ios no-auth mode was removed (security).
 */
...
if (process.env.GOTCHIBOT_OPENCODE_IOS === "1" || process.argv.includes("--ios")) {
  console.error(
    "GOTCHIBOT_OPENCODE_IOS / --ios no-auth mode was removed (security: it exposed opencode serve with no auth on the tailnet).",
  );
  process.exit(2);
}
